...
namespace WIFS
{
	...
	public class CDateTime
	{
		...
		/// </summary>
		public static DateTime DateMinValueForDb19000101
		{
		...
		/// SqlDateTime overflow. Must be between 1/1/1753 12:00:00 AM and 12/31/9999 11:59:59 PM. 
		/// </summary>
		public static DateTime DateMaxValueForDb29991231
		{
		...
		/// <returns></returns>
		public static string ToYYYYMMDD(string date, string splitChar)
		{
			if (date.Length == 8)
			{
		...
		public static string ToYYYYMMDD(string date, bool korean)
		{
			if (date.Length == 8)
			{
				if (korean)
            ...
            {
                return string.Format("{0}월{1}일",  date.Substring(0, 2), date.Substring(2, 2));
            }
		...
		/// <summary>
		/// 문자열을 날짜형식 (요일) 문자열로 변환
		///		<para>YYYYMMDD -> YYYY-MM-DD (요일)</para>
		/// </summary>
		/// <param name="date"></param>
		/// <param name="useFullName"></param>
		/// <returns></returns>
		public static string ToYYYYMMDD2(string date, bool useFullName)
		{
			return ToYYYYMMDD2(date, useFullName, false);
		}
		...
		/// </summary>
		/// <param name="date"></param>
		/// <param name="useFullName">true: 2016-08-31 (수요일) / false: 2016-08-31 (수)</param>
		/// <param name="useKorean">true: 2016년 08월 31일 false: 2016-08-31</param>
		/// <returns></returns>
		public static string ToYYYYMMDD2(string date, bool useFullName, bool useKorean)
		{
			string result = "";
			string option = "ddd"; if (useFullName) option = "dddd";

			date = ToYYYYMMDD(date);	//YYYY-MM-DD
			DateTime dt;
			if (DateTime.TryParse(date, out dt))
			{
				if (useKorean)
					result = dt.ToString("yyyy년 MM월 dd일") + " (" + dt.ToString(option) + ")";
				else
					result = date + " (" + dt.ToString(option) + ")";
			}
		...
		*/
		...
		public static string GetWeekName(DayOfWeek w)
		{
			string s = "";
			switch (w)
			{
		...
		public static string GetDayOfWeekHangul(DateTime dt)
		{
			string H = "";
			switch (dt.DayOfWeek)
			{
		...
		/// <summary>
		/// <para>지정한 날짜 문자열의 요일명을 반환한다.</para>
		/// 날짜 오류가 있을 경우 빈문자열 반환
		/// </summary>
		/// <param name="date">날짜 문자열(YYYYMMDD)</param>
		/// <param name="useFullName">전체 요일명 표시</param>
		/// <returns></returns>
		public static string GetDayOfWeekHangul(string date, bool useFullName)
		{
			string result = "";
			string option = "ddd"; if (useFullName) option = "dddd";
			DateTime dt;
			if (DateTime.TryParse(ToYYYYMMDD(date, "-"), out dt))
			{
				result = dt.ToString(option);
			}
